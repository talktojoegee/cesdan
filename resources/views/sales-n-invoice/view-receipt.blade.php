                ...
                <div class="card-body" id="receiptWrapper">
                    <div class="clearfix">
                        <div class="float-left">
                            <h3 class="card-title mb-0">#RECEIPT-{{$receipt->receipt_no ?? '' }}</h3>
                        </div>
                        <div class="float-right">
                            <p class="mb-1"><span class="font-weight-bold">Date :</span> {{date('d M, Y', strtotime($receipt->issue_date))}}</p>
                            <p class="mb-1"><span class="font-weight-bold">Ref. No. :</span> {{ strtoupper($receipt->ref_no) ?? '' }}</p>
                        </div>
                    </div>
                    <hr>
                    <div class="row">
                        <div class="col-lg-6 ">
                            <p class="h3">From:</p>
                            <address>
                                {{$receipt->getTenant->company_name ?? '' }}<br>
                                {{$receipt->getTenant->address ?? '' }}<br>
                                {{$receipt->getTenant->phone_no ?? '' }}<br>
                                {{$receipt->getTenant->email ?? '' }}
                            </address>
                        </div>
                        <div class="col-lg-6 text-right">
                            <p class="h3">To:</p>
                            <address>
                                {{$receipt->getContact->company_name ?? '' }}<br>
                                {{$receipt->getContact->company_address ?? '' }}<br>
                                {{$receipt->getContact->company_phone ?? '' }}<br>
                                {{$receipt->getContact->company_email ?? '' }}
                            </address>
                        </div>
                    </div>
                    <div class="table-responsive push">
                        <table class="table table-bordered table-hover mb-0 text-nowrap">
                            ...
                            @foreach($receipt->getDetailReceipt as $item)
                                <tr>
                                    <td class="text-center">{{$serial++}}</td>
                                    <td>
                                        <p class="font-w600 mb-1">{{$item->getServiceById($item->service_id)->item_name}}</p>
                                    </td>
                                    <td class="text-center">{{$item->quantity ?? '' }}</td>
                                    <td class="text-right number-font1">{{$item->unit_cost ?? '' }}</td>
                                    <td class="text-right number-font1">{{number_format($item->quantity * $item->unit_cost,2)}}</td>
                            ...
                            <tr>
                                ...
                                <td class="font-weight-bold text-right h4 number-font1">{{'₦'.number_format($receipt->getInvoice->getInvoiceItems->sum('total'),2)}}</td>
                            </tr>
                            <tr>
                                <td colspan="4" class="font-weight-bold text-uppercase text-right">Paid</td>
                                <td class="font-weight-bold text-right h4 number-font1">{{ '₦'.number_format($receipt->amount,2) }}</td>
                            </tr>
                            @if($receipt->receipt_type == 1)
                            <tr>
                                <td colspan="4" class="font-weight-bold text-uppercase text-right">Balance</td>
                                <td class="font-weight-bold text-right h4 number-font1">{{ '₦'.number_format($receipt->getInvoice->getInvoiceItems->sum('total') - $receipt->amount,2) }}</td>
                            </tr>
                            @endif
                            </tbody>
                        </table>
